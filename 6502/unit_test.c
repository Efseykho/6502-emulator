...
void test_sty_instr()
{
	...
	assert(emulator.Memory[0x35] == 0x20);
	
	run_program(&emulator, 2); //LDX $02, STY $35
	assert(emulator.Memory[0x35 + 0x02 ] == 0x20);
	
	run_program(&emulator, 1); //STY $01 $30
	assert(emulator.Memory[0x3001] == 0x20);
}
...
void test_sta_instr()
{
	...
	assert(emulator.Memory[0x00FA] == 0x12);
	
	run_program(&emulator, 9); 
	assert(emulator.Memory[0xEAFA] == 0x12);
	
	run_program(&emulator, 9); 
	assert(emulator.Memory[0xEB22] == 0x12);
		
	run_program(&emulator, 4); 
	assert(emulator.Memory[0xEB23] == 0x12);
		
	run_program(&emulator, 5); 
	assert(emulator.Memory[0xEB24] == 0x12);
	
	run_program(&emulator, 5); 
	assert(emulator.Memory[0xEB25] == 0x12);
}
...
void test_inc_instr()
{
	...
	//test run of SETUP_ZP_MEMORY macro
	run_program(&emulator, 3); 
	assert(emulator.Memory[0x00FA] == 0x31);
	assert( (int)(NEG_GET(emulator.P)) == 0x00 );
	assert( (int)(ZERO_GET(emulator.P)) == 0x00 );
	...
	//test run of SETUP_EXTENDED_DIRECT_MEMORY macro
	run_program(&emulator, 3); 
	assert(emulator.Memory[0xEB23] == 0xFD);
	assert( (int)(NEG_GET(emulator.P)) != 0x00 );
	
	//test run of SETUP_ABSOLUTE_INDEXED_X_MEMORY macro
	run_program(&emulator, 4); 
	assert(emulator.Memory[0xEB25] == 0xFF);
...
void test_dec_instr()
{
	...
	//test run of SETUP_ZP_MEMORY macro
	run_program(&emulator, 3); 
	assert(emulator.Memory[0x00FA] == 0x2F);
	assert( (int)(NEG_GET(emulator.P)) == 0x00 );
	assert( (int)(ZERO_GET(emulator.P)) == 0x00 );
	...
	//test run of SETUP_EXTENDED_DIRECT_MEMORY macro
	run_program(&emulator, 3); 
	assert(emulator.Memory[0xEB23] == 0xFB);
	assert( (int)(NEG_GET(emulator.P)) != 0x00 );
	
	//test run of SETUP_ABSOLUTE_INDEXED_X_MEMORY macro
	run_program(&emulator, 4); 
	assert(emulator.Memory[0xEB25] == 0xFD);
}
...
void test_cpx_instr()
{
	...
	//test run of SETUP_ZP_MEMORY macro
	run_program(&emulator, 4); 
	assert(emulator.Memory[0x00FA] == 0x30);
	assert(emulator.X == 0x07);
	assert( (int)(ZERO_GET(emulator.P)) == 0x00 );
	assert( (int)(NEG_GET(emulator.P)) != 0x00 );
	assert( (int)(CARRY_GET(emulator.P)) == 0x00 );
...
void test_rol_instr()
{
	...
	//test run of SETUP_ZP_MEMORY macro
	run_program(&emulator, 4); 
	assert(emulator.Memory[0x00FA] == 0x60);
	assert( (int)(ZERO_GET(emulator.P)) == 0x00 );
	assert( (int)(CARRY_GET(emulator.P)) == 0x00 );
	assert( (int)(NEG_GET(emulator.P)) == 0x00 );
	...
	//test run of SETUP_ZP_INDEXED_X_MEMORY macro
	run_program(&emulator, 5); 
	assert(emulator.Memory[0x00FA] == 0x61);
	assert( (int)(ZERO_GET(emulator.P)) == 0x00 );
	assert( (int)(CARRY_GET(emulator.P)) == 0x00 );
	assert( (int)(NEG_GET(emulator.P)) == 0x00 );
	...
	//test run of SETUP_EXTENDED_DIRECT_MEMORY macro
	run_program(&emulator, 4); 
	assert(emulator.Memory[0xEB23] == 0xF8);
	assert( (int)(ZERO_GET(emulator.P)) == 0x00 );
	assert( (int)(CARRY_GET(emulator.P)) != 0x00 );
	assert( (int)(NEG_GET(emulator.P)) != 0x00 );
	...
	//test run of SETUP_ABSOLUTE_INDEXED_X_MEMORY macro
	run_program(&emulator, 5);
	assert(emulator.Memory[0xEB25] == 0xFC);
...
void test_ror_instr()
{
	...
	//test run of SETUP_ZP_MEMORY macro
	run_program(&emulator, 4); 
	assert(emulator.Memory[0x00FA] == 0x18);
	assert( (int)(ZERO_GET(emulator.P)) == 0x00 );
	assert( (int)(CARRY_GET(emulator.P)) == 0x00 );
	assert( (int)(NEG_GET(emulator.P)) == 0x00 );
	...
	//test run of SETUP_ZP_INDEXED_X_MEMORY macro
	run_program(&emulator, 5); 
	assert(emulator.Memory[0x00FA] == 0x98);
	assert( (int)(ZERO_GET(emulator.P)) == 0x00 );
	assert( (int)(CARRY_GET(emulator.P)) == 0x00 );
	assert( (int)(NEG_GET(emulator.P)) != 0x00 );
	...
	//test run of SETUP_EXTENDED_DIRECT_MEMORY macro
	run_program(&emulator, 4); 
	assert(emulator.Memory[0xEB23] == 0x7E);
	assert( (int)(ZERO_GET(emulator.P)) == 0x00 );
	assert( (int)(CARRY_GET(emulator.P)) == 0x00 );
	assert( (int)(NEG_GET(emulator.P)) == 0x00 );
	...
	//test run of SETUP_ABSOLUTE_INDEXED_X_MEMORY macro
	run_program(&emulator, 5);
	assert(emulator.Memory[0xEB25] == 0x7F);
	assert( (int)(ZERO_GET(emulator.P)) == 0x00 );
	assert( (int)(CARRY_GET(emulator.P)) == 0x00 );
	assert( (int)(NEG_GET(emulator.P)) == 0x00 );
...
void test_phx_instr()
{
	...
	assert( emulator.Memory[0x01FF] == 0xCC);
	...
	emulator.P = 0xDE;
	run_program(&emulator, 1);
	assert( emulator.Memory[0x01FF] == 0xDE);
	assert( emulator.S == 0xFE);
...
void test_brk_instr()
{
	...
	emulator.Memory[0xFFFF] = 0x12;
	emulator.Memory[0xFFFE] = 0x34;
	
	run_program(&emulator, 1);
	assert( emulator.PC == 0x1234 );
	assert( emulator.Memory[0x01FF]  == 0x00 );  //high bit for the stack ptr
	assert( emulator.Memory[0x01FE]  == 0x05 );  //low bit for the stack ptr
	assert( emulator.Memory[0x01FD]  == 0x10 );  //P register with break set
	...
	emulator.Memory[0x1234] = 0x38; //SEC
	emulator.Memory[0x1235] = 0x40; //RTI
...
void test_jsr_instr()
{
	...
	//now monkey-patch in an isr to bounce us back out to somewhere sane
	emulator.Memory[0x1234] = 0x38; //SEC
	emulator.Memory[0x1235] = 0x60; //RTS
